...
<!-- AUTO_SHOP_GRID_END -->
  ...
  <script>
    (function () {
      var CATEGORY_TAB_KEY = 'massage-site-category-tab';
      function readStoredCategoryTab() {
        try {
          var v = sessionStorage.getItem(CATEGORY_TAB_KEY);
          if (v === 'shop' || v === 'outcall') return v;
        } catch (e) {}
        return 'outcall';
      }
      function writeStoredCategoryTab(mode) {
        try {
          if (mode === 'shop' || mode === 'outcall') sessionStorage.setItem(CATEGORY_TAB_KEY, mode);
        } catch (e) {}
      }
      function applyCategoryTabUI() {
        var outBtn = document.getElementById('cat-btn-outcall-static');
        var shopBtn = document.getElementById('cat-btn-shop-static');
        if (outBtn) outBtn.classList.toggle('is-active', categoryMode === 'outcall');
        if (shopBtn) shopBtn.classList.toggle('is-active', categoryMode === 'shop');
      }
      var categoryMode = readStoredCategoryTab();
      applyCategoryTabUI();
      ...
      function handleSearch() {
        ...
        if (categoryMode === 'outcall' && selectedRegion) {
          var regLc = String(selectedRegion).toLowerCase();
          var cityNorm = selectedCity ? normalizeDistrictName(selectedCity) : '';
          var subLc = selectedSub ? String(selectedSub).toLowerCase() : '';
